...
| **1.0.0-beta.2** ✓ (2026-08-04) | Schematics (`PLAN-SCHEMATICS.md`) | SPEC 16 (landed with the plan): fixed-port routing (ROUTING.md extension), capsule endpoints + typed auto-create (core), `layout: schematic` — components/pins/discretes/labels, anchor + satellite placement, junction/square-corner dress, the schematic look; the showroom (`schematic.lini`, `schematic_hero.lini` on an ISO sheet, `schematic_parts.lini`). |
