...
- **Editors** — search *Lini* in the extensions pane: a TextMate bundle on the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=monfa-red.lini) and a tree-sitter extension in [Zed's registry](https://zed.dev/extensions?query=lini). Source for both under [`editors/`](https://github.com/monfa-red/lini/tree/main/editors). Their word lists are generated from the compiler's own tables, so a new property highlights the day it lands.
