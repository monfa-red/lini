...
//! you mean" property-name hint (SPEC §19) is deferred. The pass is intentionally
//! empty for now — the home for future lints, kept so the `--no-warn` /
...
mod tests {
    ...
    fn inline_paint_is_not_linted() {
        let warns = crate::lint_str("|box#x| { fill: red; stroke: blue; }\n").expect("lint");
