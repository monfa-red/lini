...
//! class namespace, and the global block carries the scene + wire defaults plus the
...
use crate::span::Span;
use crate::syntax::ast::{Child, Decl, File, Node, Rule, SelPart, Selector, StyleItem, Wire};
use bundles::{root_defaults, wire_defaults};
use classes::{class_defs, is_lini_class, lini_class, merge_decls, worn_classes};
use std::collections::{BTreeSet, HashMap};
use types::{Types, is_template};

type Bodies = HashMap<String, (Vec<Child>, Vec<Wire>)>;
...
pub fn desugar(file: &File) -> Result<File, Error> {
    ...
    //    class order, user vars / root decls / rules, and wire-default overrides. ──
    ...
    let mut wire_user: Vec<Decl> = Vec::new();
    ...
    for item in &file.stylesheet {
        match item {
            ...
            StyleItem::Define(d) => {
                ...
                bodies.insert(d.name.clone(), (d.children.clone(), d.wires.clone()));
                push_unique(&mut extra_order, &d.name);
            }
            StyleItem::Rule(r) => match r.selector.parts.as_slice() {
                [SelPart::Type(t)] if t == "wire" => wire_user.extend(r.decls.iter().cloned()),
    ...
    // ── Lower instances, then auto-create root boxes for undeclared wire ids. ──
    let mut instances = Vec::new();
    for child in &file.instances {
        instances.push(lower_child(child, &types, &bodies)?);
    }
    let declared = scene::declared_ids(&instances);
    for (id, span) in scene::auto_created_ids(&file.wires, &declared) {
    ...
    //    scene config, vars, wire defaults, the generated `.lini-*` defs, then the
...
fn lower_node(node: &Node, types: &Types, bodies: &Bodies) -> Result<Node, Error> {
    ...
    // Wires: define-body wires (base→derived) then the node's own, each auto-along'd.
    let mut wires = Vec::new();
    if !already {
        for name in &info.chain {
            if let Some((_, body)) = bodies.get(name) {
                for w in body {
                    wires.push(labels::auto_along(w));
                }
            }
        }
    }
...
/// The `-> { }` wire-defaults rule (the wire glyph is the reserved `wire` element).
fn wire_rule(decls: Vec<Decl>) -> Rule {
    Rule {
        selector: Selector {
            parts: vec![SelPart::Type("wire".to_string())],
        },
        decls,
        span: Span::empty(),
    }
