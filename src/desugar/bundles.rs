...
pub fn primitive_bundle(kind: NodeKind) -> Vec<Decl> {
    ...
    match kind {
        ...
        // gets a sensible 1.5px border); the `|box|` template lifts paint/radius/
        // padding back on top.
        Block => vec![
            id("fill", "none"),
            id("stroke", "none"),
...
pub fn template_bundle(name: &str) -> Vec<Decl> {
    match name {
        ...
        // nodes, takes their `1.5` so its line weight matches the diagram.
        "sign" => vec![
            n("width", 64.0),
            n("height", 64.0),
            n("padding", 4.0),
...
mod tests {
    ...
    fn block_is_bare_and_box_template_carries_the_paint() {
        ...
        assert_eq!(num(&boxt, "stroke-width"), Some(1.5));
