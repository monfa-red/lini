//! The vocabulary gate [SPEC 15.11]: a floorplan scope **is** a drawing scope
//! (everything SPEC 15 gives a drawing arrives unchanged), and its own types
//! are gated to it (a `|wall|` elsewhere is an error, a `|pin|` here is one
//! exactly as it is in a drawing).

use super::{WALL, scope};
