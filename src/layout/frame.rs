...
pub(super) fn finish(
    ...
    // Chrome the cascade painted away surrenders its geometry before anything
    // measures it [SPEC 15.7] — every producer has generated by now.
    let mut nodes = nodes;
    drawing::chrome::drop_unpainted(&mut nodes);
