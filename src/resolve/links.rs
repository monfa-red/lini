...
pub fn resolve_link(
    ...
    if let Some(links) = try_projection(
        w,
        ctx,
        paths,
        path_prefix,
...
fn projection_attrs(
    ...
    let resolve_bundle =
        |decls: Vec<crate::syntax::ast::Decl>| -> Result<Vec<(String, ResolvedValue)>, Error> {
            decls
                .iter()
                .map(|d| {
                    Ok((
    ...
    if line.is_empty() {
        ordered.extend(resolve_bundle(primitive_bundle(NodeKind::Line))?);
    } else {
        ordered.extend(line);
    }
    let proj = ctx.sheet.class_decls("lini-projection");
    if proj.is_empty() {
        ordered.extend(resolve_bundle(template_bundle("projection"))?);
    } else {
        ordered.extend(proj);
    }
